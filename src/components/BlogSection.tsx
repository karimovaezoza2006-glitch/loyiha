...
import { FaSearch, FaEye, FaRegComment, FaRegHeart } from "react-icons/fa";
...
const BlogSectionInfo = () => {
 ...
 const { data: apiData = [], isLoading: apiLoading } = useQueryHandler({
  ...
  const performSearch = (term?: string, delay = 300) => {
    const q = (term ?? searchTerm).toLowerCase().trim();
    setIsSearching(true);
    setFilteredBlogs([]);
  ...
  useEffect(() => {
    ...
    debounceRef.current = window.setTimeout(() => performSearch(searchTerm), 700);
    return () => {
        ...
        <div className="mb-8 flex justify-center">
          <div className="w-full max-w-lg flex items-center gap-2 bg-gray-50 p-2 rounded-lg border border-gray-200 shadow-sm">
            <div className="relative flex-grow">
              ...
              <input
                type="text"
                placeholder="Bloglarda qidirish..."
                value={searchTerm}
            ...
            <button
              onClick={() => performSearch(searchTerm, 0)}
